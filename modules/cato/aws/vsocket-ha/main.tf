...
module "vsocket-aws-ha" {
  ...
  native_network_range_primary   = var.subnet_range_lan_primary == null ? cidrsubnet(var.vpc_range, 8, 2) : var.subnet_range_lan_primary
  native_network_range_secondary = var.subnet_range_lan_secondary == null ? cidrsubnet(var.vpc_range, 8, 3) : var.subnet_range_lan_secondary
  lan_local_primary_ip           = var.lan_eni_primary_ip == null ? cidrhost(cidrsubnet(var.vpc_range, 8, 2), 5) : var.lan_eni_primary_ip
  lan_local_secondary_ip         = var.lan_eni_secondary_ip == null ? cidrhost(cidrsubnet(var.vpc_range, 8, 3), 5) : var.lan_eni_secondary_ip
  mgmt_subnet_id                 = module.vpc.mgmt_subnet_id
  wan_subnet_id                  = module.vpc.wan_subnet_id
